...
enum biometric_attribute {
	...
	/** Security level (0-10000), higher values reduce false accepts */
	BIOMETRIC_ATTR_SECURITY_LEVEL,
	/** Default operation timeout in milliseconds */
	BIOMETRIC_ATTR_TIMEOUT_MS,
	/** Anti-spoofing detection sensitivity (0-10000) */
 ...
 * @param template_id Template ID to assign
 *
 * @retval 0 on success
...
static inline int z_impl_biometric_enroll_capture(const struct device *dev, k_timeout_t timeout)
{
	...
	return api->enroll_capture(dev, timeout);
}
						 ...
						 size_t max_count, size_t *actual_count)
{
	...
	if (dev == NULL || ids == NULL || actual_count == NULL) {
		return -EINVAL;
	}
 ...
 *
 * For IDENTIFY mode, retrieve the matched template ID using
 * biometric_attr_get(BIOMETRIC_ATTR_PRIV_START).
 *
 * @param dev Biometric device
 ...
 *
 * @retval >=0 Confidence score (raw sensor value, higher is better)
 * @retval -ETIMEDOUT Timeout waiting for sample
					 ...
					 uint16_t template_id, k_timeout_t timeout)
{
	...
	return api->match(dev, mode, template_id, timeout);
}
