...
static int cmd_biometric_enroll_start(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (template_id < 0 || template_id > UINT16_MAX) {
		shell_error(sh, "Template ID out of range (0-%d)", UINT16_MAX);
		return -EINVAL;
	}
...
/* clang-format off */
SHELL_STATIC_SUBCMD_SET_CREATE(sub_biometric,
...
SHELL_CMD_REGISTER(biometric, &sub_biometric, "Biometric commands", NULL);
