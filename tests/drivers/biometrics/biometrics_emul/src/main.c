...
ZTEST(biometrics_emul, test_attr_set_get)
{
	...
	ret = biometric_attr_set(dev, BIOMETRIC_ATTR_SECURITY_LEVEL, 5000);
	zassert_equal(ret, 0, "Failed to set security level: %d", ret);

	ret = biometric_attr_set(dev, BIOMETRIC_ATTR_SECURITY_LEVEL, 15000);
	zassert_equal(ret, -EINVAL, "Should reject out-of-range security level");

	ret = biometric_attr_set(dev, BIOMETRIC_ATTR_SECURITY_LEVEL, -1);
	zassert_equal(ret, -EINVAL, "Should reject negative security level");
...
ZTEST(biometrics_emul, test_enrollment_flow)
{
	...
	for (int i = 0; i < caps.enrollment_samples_required; i++) {
		ret = biometric_enroll_capture(dev, K_SECONDS(5));
...
ZTEST(biometrics_emul, test_match_verify)
{
	...
	ret = biometric_match(dev, BIOMETRIC_MATCH_VERIFY, 300, K_SECONDS(5));
	zassert_true(ret > 0, "Match should succeed with score: %d", ret);
	zassert_equal(ret, 85, "Expected score 85, got %d", ret);
...
ZTEST(biometrics_emul, test_match_identify)
{
	...
	ret = biometric_match(dev, BIOMETRIC_MATCH_IDENTIFY, 0, K_SECONDS(5));
	zassert_true(ret > 0, "Identify should succeed: %d", ret);
	zassert_equal(ret, 92, "Expected score 92");

	ret = biometric_attr_get(dev, BIOMETRIC_ATTR_PRIV_START, &matched_id);
	zassert_equal(ret, 0, "Failed to get matched ID");
	zassert_equal(matched_id, 401, "Expected matched ID 401, got %d", matched_id);
...
ZTEST(biometrics_emul, test_capture_timeout)
{
	...
	ret = biometric_enroll_capture(dev, K_SECONDS(1));
...
ZTEST(biometrics_emul, test_null_pointer_handling)
{
	const struct device *dev = get_biometrics_device();
	int ret;

	ret = biometric_get_capabilities(NULL, NULL);
	zassert_equal(ret, -EINVAL, "Should reject NULL device");

	ret = biometric_get_capabilities(dev, NULL);
	zassert_equal(ret, -EINVAL, "Should reject NULL caps");

	ret = biometric_attr_get(dev, BIOMETRIC_ATTR_MATCH_THRESHOLD, NULL);
	zassert_equal(ret, -EINVAL, "Should reject NULL value pointer");

	ret = biometric_template_store(dev, 0, NULL, 10);
	zassert_equal(ret, -EINVAL, "Should reject NULL data pointer");

	ret = biometric_template_list(dev, NULL, 10, NULL);
	zassert_equal(ret, -EINVAL, "Should reject NULL list pointers");
}
